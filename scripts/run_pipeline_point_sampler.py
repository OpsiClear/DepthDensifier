...
class SamplingConfig:
    ...
    """Weight for edge-based sampling (0.3 random, 0.7 gradient-based)."""
    use_gpu: bool = True
...
def process_batch(
    ...
    for idx, image in enumerate(batch_images):
        ...
        sample_device = "cuda" if config.sampling.use_gpu and torch.cuda.is_available() else "cpu"
        ...
        for i, (x, y) in enumerate(sampled_points):
            ...
            if 0 <= x_int < w and 0 <= y_int < h and refined_depth[y_int, x_int] > 0:
                ...
                valid_colors.append(sampled_colors[i] if sampled_colors is not None else img_np[y_int, x_int])
        ...
        logger.info(f"Image {image.image_id}: Sampled {len(valid_points)}/{config.sampling.num_points_per_image} valid points "
...
def main(config: ScriptConfig):
    ...
    logger.info(f"Edge weight: {config.sampling.edge_weight} (Random: {1-config.sampling.edge_weight:.1%}, Gradient: {config.sampling.edge_weight:.1%})")
    logger.info(f"GPU sampling: {config.sampling.use_gpu and torch.cuda.is_available()}")
    ...
    all_images = [(img, config.paths.image_dir / img.name) 
    ...
    if config.processing.skip_every_n_images > 1:
        image_list = [img for i, img in enumerate(all_images) 
                      if i % config.processing.skip_every_n_images == 0]
        logger.info(f"Skipping images: processing every {config.processing.skip_every_n_images} image(s)")
    ...
    logger.info(f"Using point sampling instead of grid-based approach")
