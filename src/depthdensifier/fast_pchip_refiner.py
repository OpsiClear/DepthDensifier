...
class FastPCHIPRefiner:
    ...
    def _detect_depth_edges(
        self, depth_map: np.ndarray, mask: np.ndarray | None, normal_map: np.ndarray | None
    ) -> np.ndarray:
        ...
        if normal_map is not None and normal_map.shape[-1] == 3:
            ...
            normal_grad_mag = np.sqrt(
                nx_grad[0]**2 + nx_grad[1]**2 +
                ny_grad[0]**2 + ny_grad[1]**2 +
                nz_grad[0]**2 + nz_grad[1]**2
            )
    ...
    def _pchip_interpolate_torch(
        ...
        result = torch.where(depths_torch <= x_torch[0], 
                            y_torch[0] + slope_left[0] * (depths_torch - x_torch[0]) * 0.3,
                            result)
        result = torch.where(depths_torch >= x_torch[-1],
    ...
    def refine_depth(
        ...
        if self.use_image_edges and rgb_image is not None:
            ...
            if self.verbose > 0:
                print(f"[FastPCHIP] Using image-based edge detection (threshold={self.image_edge_threshold})")
        else:
            ...
            if self.verbose > 0 and self.use_image_edges:
                print("[FastPCHIP] Warning: use_image_edges=True but no RGB image provided, falling back to depth edges")
        ...
        if len(z_depth) < self.min_correspondences:
            if self.verbose > 0:
                print(f"[FastPCHIP] Too few correspondences ({len(z_depth)} < {self.min_correspondences})")
            return {
                "refined_depth": depth_map,
                "scale": 1.0,
                "energy_history": [],
        ...
        if self.robust:
            z_colmap_clean, z_depth_clean, n_outliers = self._remove_outliers(z_colmap, z_depth)
    ...
    def _apply_edge_aware_transformation(
        self, depth_map: np.ndarray, mask: np.ndarray, edge_mask: np.ndarray,
        ...
        if non_edge_mask.any():
            refined_depth = self._apply_pchip_transformation(depth_map, non_edge_mask, x, y)
        
        # For edge pixels, apply conservative blending
        edge_valid = mask & edge_mask
        if edge_valid.any():
            # Apply transformation but blend with original
            edge_transformed = self._apply_pchip_transformation(depth_map, edge_valid, x, y)
            # Blend: 70% original, 30% transformed for edge pixels
            refined_depth[edge_valid] = (
                0.7 * depth_map[edge_valid] + 
                0.3 * edge_transformed[edge_valid]
            )
